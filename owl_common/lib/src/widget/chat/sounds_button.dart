...
import 'package:owl_common/src/utils/chat_data.dart';
import 'package:owl_common/src/utils/recorder.dart';
...
class SoundsMessageButton extends StatefulWidget {
  ...
  /// 发送音频 / 发送音频文字
  final Function(SendContentType type, String content, Duration duration)? onSendSounds;
          ...
          child: RecordingStatusMaskView(
            ...
            onVoiceSend: () {
              widget.onSendSounds?.call(
                  SendContentType.voice, _soundsRecorder.path.value ?? '',_soundsRecorder.duration.value );
              _removeMask();
            },
            onTextSend: () {
              widget.onSendSounds?.call(SendContentType.text,
                  _soundsRecorder.textProcessedController.text,_soundsRecorder.duration.value);
              _removeMask();
            },
          ),
        );
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                ...
                content: "chat_record_no_permission".tr.toText,
              ),
            );
          }
          return;
        ...
        _soundsRecorder.beginRec(
          ...
          onCompleted: (path, duration) {
              ...
              showDialog(
                ...
                builder: (context) {
                  return CupertinoAlertDialog(
                    title: "chat_record_short_time".tr.toText,
                    actions: [
                      CupertinoDialogAction(
                        child: "confirm".tr.toText,
                        onPressed: () {
            ...
            if (_soundsRecorder.status.value == SoundsMessageStatus.recording) {
              widget.onSendSounds?.call(SendContentType.voice, path!, _soundsRecorder.duration.value);
              _removeMask();
            }
            // 取消发送
      ...
      child: ValueListenableBuilder(
        ...
        builder: (context, value, child) {
          ...
          return Container(
            ...
            child: "chat_voice_button_hint".tr.toText
